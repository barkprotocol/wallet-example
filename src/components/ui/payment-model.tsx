...
const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, paymentMethods, currencies }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<PaymentFormData>({
    defaultValues: {
      ...
      paymentMethod: paymentMethods[0]?.id || '', // Handle cases where paymentMethods might be empty
      description: '',
    },
  });
      ...
      <div className="min-h-screen px-4 text-center">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
            ...
            <PaymentProcessing isProcessing={isProcessing} isSuccess={isSuccess} error={errorMessage} />

            <div className="mt-4 flex justify-end space-x-2">
              <PayButton onClick={onClose} isLoading={isProcessing}>
                Cancel
              </PayButton>
              <PayButton type="submit" isLoading={isProcessing} error={errorMessage} success={isSuccess}>
                {isProcessing ? 'Processing...' : isSuccess ? 'Success!' : 'Submit Payment'}
              </PayButton>
            </div>
          </form>
        </div>
